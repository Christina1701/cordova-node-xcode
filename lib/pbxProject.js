...
pbxProject.prototype.addResourceFile = function(path, opt, group) {
    ...
    if (!opt.plugin) {
        ...
        if (group) {
            if (group === 'App') {
                this.addToPbxGroupApp(file);                // "App" Group
            } else if (this.getPBXGroupByKey(group)) {
            ...
            this.addToResourcesPbxGroup(file);              // PBXGroup (as new own section)
        }
    }
 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addAppEntitlementsFile = function(path, opt, group) {
    opt = opt || {};
    var file;
    ...
    this.addToPbxFileReferenceSection(file);        // PBXFileReference
    if (group === 'App') {
        const appKey = this.getPBXGroupKeyForApp();
        this.addToPbxGroup(file, appKey);           // "App" Group
    }

    // add to correct XCBuildConfiguration Debug & Release
    this.addEntryToPbxXCBuildConfigurationSections('CODE_SIGN_ENTITLEMENTS', `App/${path}`);

    return file;
};

/**
 * sets a property with corresponding value in required pbxBuildConfigurationsSections´ "buildSettings" property
 * @param property {string} property to set in PbxXCBuildConfigurationSection
 * @param value {string} value to set for property
 */
pbxProject.prototype.addEntryToPbxXCBuildConfigurationSections = function(property, value) {
    const buildConfigSections = this.getPbxXCBuildConfigurationSections();
    for (key in buildConfigSections) {
        const buildSettings = buildConfigSections[key]['buildSettings'];
        buildSettings[property] = value;
    }
};

/**
 * filter all pbxBuildConfigurationsSections to required ones (those with "baseConfigurationReference") to add CODE_SIGN_ENTITLEMENTS, DEVELOPMENT_TEAM, INFOPLIST_FILE, SWIFT_ACTIVE_COMPILATION_CONDITIONS etc.
 * @returns {Array} required pbxBuildConfigurationsSections
 */
pbxProject.prototype.getPbxXCBuildConfigurationSections = function() {
    const buildConfigSections = this.pbxXCBuildConfigurationSection();
    const correctBuildConfigurationSections = [];

    for (let key in buildConfigSections) {
        if (buildConfigSections[key].hasOwnProperty('baseConfigurationReference') && buildConfigSections[key].hasOwnProperty('buildSettings')) {
            correctBuildConfigurationSections.push(buildConfigSections[key]);
        }
    }

    if (correctBuildConfigurationSections.length <= 0) {
        throw new error('No PbxXCBuildConfigurationSections found!');
    }

    return correctBuildConfigurationSections;
};
...
pbxProject.prototype.addToPbxGroupApp = function(file) {
    const appGroupKey = this.getPBXGroupKeyForApp();
    this.addToPbxGroup(file, appGroupKey);      // "App" Group
};
...
pbxProject.prototype.getPBXGroupKeyForApp = function() {
    let keyForApp;

    for (let [key, value] of Object.entries(this.hash.project.objects['PBXGroup'])) {
        if (value === 'App') {
            keyForApp = key.replace('_comment', '');
        }
    }
